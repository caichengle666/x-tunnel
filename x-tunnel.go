...
func newSOCKS5UDPRelay(targetAddr string) (*SOCKS5UDPRelay, error) {
	...
	if relayHost == "0.0.0.0" || relayHost == "::" {
		h, _, errSplit := net.SplitHostPort(socks5Config.Host)
		if errSplit != nil || h == "" {
			h = strings.TrimSpace(socks5Config.Host)
		}
		if h == "" {
			tcpConn.Close()
			return nil, errors.New("SOCKS5 UDP 中继无法确定代理地址")
		}
		relayHost = h
	}
	if relayHost == "" {
		tcpConn.Close()
		return nil, errors.New("SOCKS5 UDP 中继地址为空")
	}
...
func parseDNSResponse(response []byte) (string, error) {
	...
	offset := 12
	// 跳过 Question 段中的查询名称（支持 DNS 压缩指针）
...
func handleSmuxStream(session *ClientSession, ch *WSChannel, stream *smux.Stream) {
	...
	case streamKindTCP:
		...
		if err != nil {
			log.Printf("[服务端] TCP 拨号失败 %s: %v", target, err)
			// 回复 1 字节错误标记让客户端知道失败原因
	...
	case streamKindUDP:
		...
		if socks5Config != nil {
			...
			if errResolve != nil {
				log.Printf("[服务端] UDP 地址解析失败 %s: %v", target, errResolve)
				_, _ = stream.Write([]byte{0x01})
				return
			}
			udpConn, errListen := net.ListenUDP("udp", nil)
			if errListen != nil {
				log.Printf("[服务端] UDP 监听失败: %v", errListen)
				_, _ = stream.Write([]byte{0x01})
				return
			}
...
func readTunnelStatus(stream *smux.Stream) error {
	...
	if status[0] != 0x00 {
		return errors.New("tunnel rejected by server")
	}
	return nil
}
...
func (a *UDPAssociation) send(target string, data []byte) {
	...
	} else if target != "" && target != a.target {
		// 切换目标：关闭旧 stream，新建
		if stream != nil {
			_ = stream.Close()
		}
